...
const updateKeyCodeText = () => {
  //键值设置
  var keyNum = 8
  const keyInfoArr = [...document.getElementById('btnSettingsList').getElementsByClassName('midP')]
...
const initSettings = () => {
  ...
  delayInput.value =
    settingsSet[9][0] * 0x01000000 + settingsSet[9][1] * 0x010000 + settingsSet[9][2] * 0x0100 + settingsSet[9][3]
  //极速模式
  if (settingsSet[10][0]) {
  ...
  // 自动连击启用
  for (let i = 1; i <= 8; i++) {
    document.getElementById(`autoClickEnable_BT${i}`).checked = !!(settingsSet[11][0] & (0x80 >> (8 - i)))
  }
  // 自动连击延迟
  const autoClickDelayInput = document.getElementById(`autoClickDelayInput`)
  autoClickDelayInput.value = (settingsSet[11][2] << 8) | settingsSet[11][3]
...
const initSettingsFunction = () => {
  ...
  document.getElementById('delayInput').addEventListener('change', (e) => {
    var value = parseInt(document.getElementById('delayInput').value).toString(16).padStart(8, '0')
  ...
  // 自动连击启用情况
  for (let i = 1; i <= 8; i++) {
    document.getElementById(`autoClickEnable_BT${i}`).addEventListener('change', (e) => {
      console.log(settingChanged[11][0], i, 0x80 >> (8 - i))
      if (document.getElementById(`autoClickEnable_BT${i}`).checked) {
        settingChanged[11][0] |= 0x80 >> (8 - i)
      } else {
        settingChanged[11][0] &= 0xff - (0x80 >> (8 - i))
      }
      console.log(settingChanged[11][0], i)
      countChanges()
    })
  }
  // 自动连击延迟
  const autoClickDelayInput = document.getElementById(`autoClickDelayInput`)
  autoClickDelayInput.addEventListener('change', (e) => {
    settingChanged[11][2] = (autoClickDelayInput.value & 0xff00) >> 8
    settingChanged[11][3] = autoClickDelayInput.value & 0x00ff
    countChanges()
  })
  // 键值设置
  var keyNum = 8
  const keyInfoArr = [...document.getElementById('btnSettingsList').getElementsByClassName('rightP')]
  ...
  const updateKeyBoard = () => {
    ...
    keyPosSet.forEach((keys) => {
      ...
      addClassName(
        keyboardPannel.getElementsByClassName('keyboard-line')[line - 1].getElementsByClassName('key')[col - 1],
        'select'
      )
    })
  }
  ...
  for (let keys in KeyData) {
    const line = parseInt(keys.substr(0, 1))
    const col = parseInt(keys.substr(1, 2))
    keyboardPannel.getElementsByClassName('keyboard-line')[line - 1].getElementsByClassName('key')[
      col - 1
    ].dataset.keyKey = keys
  }
  ;[...keyboardPannel.getElementsByClassName('key')].forEach((nodeEle) => {
...
// 默认数据
const templeData = [
  [0x01, 0x00, 0x1e, 0x00, 0x00], //1
  [0x02, 0x00, 0x1f, 0x00, 0x00], //2
  [0x03, 0x01, 0x20, 0x00, 0x00], //3
  [0x04, 0x00, 0x21, 0x00, 0x00], //4
  [0x05, 0x00, 0x22, 0x00, 0x00], //5
  [0x06, 0x00, 0x23, 0x00, 0x00], //6
  [0x07, 0x00, 0x24, 0x00, 0x00], //7
  [0x08, 0x00, 0x25, 0x00, 0x00], //8
  [0x09, 0x00, 0x00, 0x00, 0x80], //0x80 => 128 (MAX 16^6-1)
  [0x0a, 0x01, 0x00, 0x00, 0x40], //0x01 极速模式处于开启
  [0x0b, 0x01, 0x00, 0x00, 0x40], //0x01 极速模式处于开启
]
...
const funs = (documentElement, deviceObj, funs) => {
  ...
  document.getElementById('useSettings').addEventListener('click', (e) => useSettings())
  ...
  const leftPannelSpans = [...document.getElementById('leftPannel').getElementsByTagName('span')].slice(0, 5)
  ...
  const leftPannelScrollTo = (i, t = 300) => {
    ...
    raf = requestAnimationFrame(function autoRun(t) {
      ...
      rightPannel.scrollTop = easeOut((t - timeStart) / timeLong) * scrollGo + scrollFrom
      checkScroll()
  ...
  document.getElementById('checkROMUpdate').addEventListener('click', (e) => {
    if (!checkROMUpdateBoolean) {
      ...
      checkROMUpdateTimer = setTimeout(() => (checkROMUpdateBoolean = false), 5000)

      document.getElementById('checkROMUpdate').innerText = getLang('pleaseWait')
      const xhr = new XMLHttpRequest()
      xhr.open('GET', encodeURI('http://182.254.136.143:8117/check-roms/' + deviceInfo.description + '.json'))
...
Update Now?`)
                  ...
                  setTimeout(() => {
                    const execFile = require('child_process').execFile
                    const cmd = execFile(path.join(__dirname, '/UPDATER/iap_applition.exe'), [], {
                      // detached: true,
                      // stdio: 'ignore',
                      //shell: true,
                    ...
                    httpObj.get(obj.url, (res) => {
                      ...
                      res.on('end', () => {
                        filePath = path.join(
                          os.tmpdir(),
                          crypto.createHash('md5').update(fileData).digest('hex') + '.hex'
                        )
                    ...
                    cmd.stdout.on('data', (data) => {
                      ...
                      if (dataStr.indexOf('READY') > -1) {
                        ...
                        alert('SUCCESS')
                        cmd.kill(cmd.pid)
                      } else if (dataStr.indexOf('WRONG_FAILED_TO_DOWNLOAD') > -1) {
                        alert('FAILED')
                        cmd.kill(cmd.pid)
                      }
                    })
                ...
                // DO something
                document.getElementById('checkROMUpdate').innerText = getLang('isLatestVersion')
              }
              // alert(
        ...
        if (!succ) {
          document.getElementById('checkROMUpdate').innerText = getLang('checkUpdateFailed')
        }
      })
      xhr.send()
    }
  ...
  Array.prototype.forEach.call(document.querySelectorAll("input[type='color']"), (e) => {
    e.addEventListener('click', (e) => e.preventDefault())
  })
  document.getElementById('theBtnDefInner').style.background = `url("${
    __dirname.replace(/\\/g, '/') + '/imgs/deviceKeyInfo.png'
  }")`
  // document.getElementById(
  //   'theLightDefInner'
  // ).style.background = `url(${__dirname.replace(/\\/g, '/') +
  //   '/imgs/deviceLightInfo.png'})`
  document.getElementById('deviceKeyMapTitle').innerText = getName(deviceInfo.description)
